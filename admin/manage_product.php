	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			if($_FILES['image']['name']!=''){
				...
				$update_sql="update product set categories_id='$categories_id',name='$name',mrp='$mrp',price='$price',qty='$qty',description='$description',image='$image',sub_categories_id='$sub_categories_id' where id='$id'";
			}else{
			...
			$image=rand(111111111,999999999).'_'.$_FILES['image']['name'];
			move_uploaded_file($_FILES['image']['tmp_name'],'../media/product/'.$image);
			mysqli_query($con,"insert into product(categories_id,name,mrp,price,qty,description,status,image,sub_categories_id,added_by) values('$categories_id','$name','$mrp','$price','$qty','$description',1,'$image','$sub_categories_id','".$_SESSION['ADMIN_ID']."')");
		}
		header('location:product.php');
		die();
	} 
                        ...
                        <form method="post" enctype="multipart/form-data">
							<div class="card-body card-block">
							   <div class="form-group">
								...
								<div class="form-group">
									<label for="categories" class=" form-control-label">New Price</label>
							   ...
							   <button style="background:#151c58" id="payment-button" name="submit" type="submit" class="btn btn-lg btn-info btn-block">
